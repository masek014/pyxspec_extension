"""
Establishes a simple interface to set XSPEC parameters since some parameters
...
from . import DEFAULT_PARAMETER_FILE, DEFAULT_PILEUP_PARAMETER_FILE
...
class XSPECConfiguration():
    ...
    def _convert_to_xspec_units(self):
        ...
        for (param, values) in self.conf_dict.items():
            if param == 'kT':
                self.conf_dict[param] = {k: (v / KEV_TO_KELVIN).value for (k,v) in values.items()}
            elif param == 'norm':
                self.conf_dict[param] = {k: (v / EARTH_NORM_TO_CM3).value for (k,v) in values.items()}
            elif param == 'BreakE':
                self.conf_dict[param] = {k: v.to(u.keV).value for (k,v) in values.items()}
            elif 'PhoIndx' in param:
                self.conf_dict[param] = {k: v.value for (k,v) in values.items()}


    def _read_file(self):
        
        with open(self.conf_file, 'r') as in_file:
                ...
                if line[0] != '#' and line[0] != '\n':
                    ...
                    if p is not None:
                        ...
                        try:
                            ...
                            print(f'Key \'{param}\' not defined in the XSPEC parameter map.\n{e}')
                        unit = values.pop('unit').replace('[]', '')
                        values = {k: float(values[k]) * u.Unit(str(unit)) for k in values.keys()}
                        self.conf_dict[xspec_param] = {**values}
                    else:
                        line = line.replace('\n', '')
                        raise ValueError(f'Provided configuration file \'{self.conf_file}\' contains an incorrectly formatted line: \'{line}\'\nCompare against the default configuration file found at {DEFAULT_PARAMETER_FILE} for proper formatting.')

        self._convert_to_xspec_units()


def _test() -> XSPECConfiguration:
    return XSPECConfiguration()